...
// adjust meta outputs for logo and additional notes
const processMeta = require( './Meta' )
// generatePage
...
class Init {
    ...
    initInput = function( emType ) {
        inquirer
            ...
            .then( data => {
                // store role from selection
                data.role = emType
                // update meta with logo and expanded additional data
                data.meta = processMeta( data )
                // send data to generator
                const employee = new Generator( data )
                employee.str = employee.generateString()
                // put to storage array
