            ...
            .prompt( [{
                ...
                validate: input => { // text validate that an input has been entered
                    if( input ) {
                        ...
                        console.log( ``) // console log to start new line
                        console.log( `Enter ${ emType } name` ) // error
                        return false
                    }
                }
            },
            {
                ...
                validate: input => { // number greater than zero validate
                    if( !input === NaN || input > 0) {
                        const checkVal = obj => obj.id === input
                        if( employeesArr.some( checkVal ) ) { // check to see if id already exists in data
                            console.log( ``)
                            console.log( `id number ${input} is already assigned`)
                            return false
                        } else {
                            return true
                        }
